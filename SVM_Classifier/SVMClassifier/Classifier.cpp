...
Classifier::Classifier(unique_ptr<ImageProcessorBase> _imageProcessor, Mat bowDictionary, string svmPath) :Classifier(move(_imageProcessor), bowDictionary)
{
...
void Classifier::VisualizeClassification(const vector<float>& results) const
{
...
void Classifier::VisualizeBOWClassification(vector<float> predictions) const
{
...
void Classifier::TrainSvm()
{
	...
	params.gamma = 0.000000001; // for poly/rbf/sigmoid

	cout << "Training svm" << endl;
	svm->train(svmData, svmLabels, Mat(), Mat(), params);

	cout << "Saving svm";
	svm->save("HSV_cylinder_only.xml");


	try
	{
		cout << "Saving data for later :)";	
		FileStorage fs("svmData_HSV_cylinder_only.xml", FileStorage::WRITE);
		fs << "svmData_HSV_cylinder_only" << svmData;
		fs.release();

		fs = FileStorage("svmLabel_HSV_cylinder_only.xml", FileStorage::WRITE);
		fs << "svmLabel_HSV_cylinder_only" << svmLabels;
		fs.release();
...
void Classifier::VisualizeClassificationRect(const vector<float>& results)
{
}
